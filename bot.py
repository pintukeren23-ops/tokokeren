...
async def create_qris_payment(amount, description, order_id):
    """Buat transaksi QRIS baru via Cutiezy API. Return data transaksi atau None kalau gagal."""
    try:
        async with aiohttp.ClientSession() as session:
            ...
            headers = {
                "X-API-Key": CUTIEZY_API_KEY,
                "Content-Type": "application/json"
            }
            async with session.post(
                CUTIEZY_BASE_URL + "/create-payment",
                json=payload,
                headers=headers,
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(
                CUTIEZY_BASE_URL + "/check-payment/" + transaction_id,
                headers=headers,
                timeout=aiohttp.ClientTimeout(total=10)
            ) as resp:
                data = await resp.json()
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "batal":
        ...
        await query.edit_message_text("Membuat QRIS pembayaran... mohon tunggu sebentar.")

        # Buat transaksi QRIS via Cutiezy API
        ...
        if not trx_data:
            await query.edit_message_text(
                "Gagal membuat QRIS. Silakan coba lagi atau hubungi admin @" + ADMIN_USERNAME
            )
            return
        ...
        # Simpan transaction_id untuk tracking status
        ...
        try:
            ...
            pass
        # Kirim gambar QR dari URL Cutiezy langsung
        async with aiohttp.ClientSession() as session:
            async with session.get(qris_image_url) as img_resp:
        ...
        if status == "paid":
            # Pembayaran berhasil — notif admin
            caption = (
            ...
            await query.answer("Pembayaran belum diterima. Pastikan kamu sudah scan dan bayar, lalu cek lagi.", show_alert=True)
        elif status in ("expired", "cancel"):
            await query.answer("QRIS sudah expired/dibatalkan. Silakan order ulang.", show_alert=True)
            try:
                await query.delete_message()
            except Exception:
                pass
        else:
            await query.answer("Gagal cek status. Coba lagi atau hubungi admin.", show_alert=True)
        ...
        nomor = parts[2]
        # Simpan state ke database (bukan context.user_data) supaya tahan restart
