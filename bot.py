...
async def create_qris_payment(amount, description, order_id):
    ...
    def _call():
        try:
            # Tambah order_id % 99 ke nominal biar unik (bypass duplicate pending error)
            # Contoh: order #5, Rp150.000 → Rp150.005
            unique_amount = amount + (order_id % 99)
            payload = {"amount": unique_amount, "description": description}
